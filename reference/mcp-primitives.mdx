...
fragment, or a username and password. Every address Postern publishes is served at the
root. Postern would then advertise a saved path to agents as an address that answers
nothing. Meanwhile the boot log, both discovery documents and the Console's own
indicator would all still read correctly. The refusal names the origin to save instead. A port is fine:
Postern drops a default port, so `https://host.ts.net:443` is stored as
...
Field lists are not repeated here. Call `get_schema`: it is the authority, and it is
filtered to your own grants. Read `notes` and `fieldDocs` — they carry the sign
...
strings, or `null` when the aggregated set was empty. Combining `aggregate` with
`select`, `orderBy` or `offset` is refused, and the refusal names the field.

Rows come back ordered by the object's `primaryTime` descending, with `id` ascending as
the tiebreak, unless you order them yourself. Ordering is deterministic within one
...
is opt-in because selecting `raw` costs a fetch and a decompress per row.
...
one message at a time, live from Apple. Changing the window clears the marker of how far the
connection has read, so the next run re-reads the whole window from the start. On a
large mailbox that is minutes of work, and **Envelopes cached** falls before it climbs.
...
of days late. Re-reading the overlap changes nothing already stored.
...
dropped from the catalog and refused again at dispatch. Naming one directly does not
get around the omission: the answer is
