...
means never. A key is dead at the instant it expires, even inside a session that
is already open. [Expiry](/reference/grants-and-sectors#expiry) carries the
...
stops a key that never expires, because Postern cannot know then that the key will
go to someone else's cloud. So the bridge refuses a key you created before you had
a public address — at the sign-in, not earlier. A key that never expires, held by a
company's cloud, ends only when you notice it.
...
Nothing at the source changes. Revoke a hosted agent's key and Postern refuses its
next call. Rotate, and its passes die at the same moment. A rotation is the answer
to a leaked key, which is why the derived credentials die with it.
...
length outright, and never pads or truncates one. There is no passphrase, no key derivation
...
secret from the vault. The cached rows are a third thing, and Postern leaves them
alone.
...
| An agent key that worked yesterday is refused today, with nothing changed | It has usually met its expiry. Console → **Agents & keys** → the agent shows its deadline. Rotate to create a fresh key, then hand the new one to the agent. |
