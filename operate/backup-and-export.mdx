...
<Info>
  **Before you start**
  ...
  - **The Console cannot back up.** Settings → Your data has a **Back up now** row. Its **Back up**
    button is disabled: backup runs on the machine, and the Console is not told how it went.
  - **Postern never backs up on its own.** You decide when to run this, and where the copy goes.
  - **These commands are written for macOS and Linux.** Postern itself runs on Windows, because
    it runs under Docker — but nobody has tested it there, and this page has no PowerShell form.
</Info>
...
**Postern holds it.** Postern made a 32-byte key on first boot and keeps it at
`/app/.pci/master.key`, inside a Docker storage area named `pci_master_key`. The storage area is
the key.
...
- **One connection** — that source's cached rows only. It stays connected, and a sync fills it
  again.
...
2. Type `ERASE` into the **Type ERASE to confirm** box.
...
Running the same erase twice is safe. An erase that stops partway is fully recovered by running the
same command again. [Why](/reference/security-model)
