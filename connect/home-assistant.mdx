...
In Home Assistant, click your own name at the very bottom of the left sidebar. Your profile opens.
Home Assistant's own documentation puts long-lived access tokens at the bottom of that profile's
Security tab — [Managing account access](https://www.home-assistant.io/docs/authentication/#managing-account-access).
Older Home Assistant versions show no tabs. There the same section sits at the bottom of the
profile page.
...
{/* UNWITNESSED: nobody has captured Home Assistant's profile page. Needed before any label here is bolded — the tab strip at the top of the profile, the exact label of the control that creates a token, the name field's label, and the reveal screen. Until a capture arrives this step names the outcome and the location and cites Home Assistant's own documentation. */}

Create a long-lived access token there, and name it `Postern`. That name is how you tell this token
apart from your others in Home Assistant.

The token appears in full. Copy it into your password manager. You paste it into the Console in
step 3, and again whenever you reconnect.
...
<Note>
  **Unreachable**, with *"can't reach source — retrying"*: nothing answered at that address. Postern
  tries again after 0.5 seconds, then doubles the wait after each failure, up to 30 seconds. A Home
...
| **Error**, `auth failed: invalid token` | Create a new token in Home Assistant, paste it on the same card, and press **Reconnect Home Assistant**. Take the same path if you lost the token before it reached the Console. |
...
{/* UNWITNESSED: the control that removes a token from Home Assistant's own list. Needed — a capture of a token row on the profile's Security tab, its remove affordance, and any confirmation it opens. */}

To stop it at Home Assistant, remove the token from the same profile page you created it on. The
connection then fails at its next check with `auth failed: invalid token`, and stays in error until
you paste a new one.
