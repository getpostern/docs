...
{/* WITNESSED: gif-whoop.md — the four sections below CONTACTS, their helper text, the six scope checkboxes, and the submit button. Casing caveat from that witness: the ALL-CAPS headings and button labels are probably a CSS text-transform, so the render is certain and the source string is not. Describe what renders; never assert the underlying string. */}
...
will take the form of https://whoop.com or whoop://example." Your address matches neither of those
...
{/* UNWITNESSED: whether WHOOP's app screen offers a control that issues a replacement client secret. The founder's capture of that screen (founder-round2.md §2.2) shows Client ID in full and Client Secret masked; it settles nothing about a replacement control. Do not name one. */}
