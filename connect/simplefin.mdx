...
`Removes Finance's cached data and this connection. Re-add it anytime.` Type `disconnect` in the box
under **Type disconnect to confirm**, then press **Disconnect Finance**. Postern deletes this
...
arrives through both, but only when both publish the last 4 digits. Otherwise it asks you to settle
...
<Columns cols={2}>
  <Card title="Connect an agent" href="/start/connect-an-agent">
    a key you created, and a client that can reach the machine Postern runs on · a few minutes, plus
    a restart of that client
  </Card>
