...
so Claude, ChatGPT and Cursor all connect the same way.
...
successfully, and Postern then denies every read. The mint form warns you under the button: `No
...
the agent's next session.` That cuts the agent's live session, and the new sectors
apply when it connects again. The key itself does not change, so you paste nothing
a second time. [Read a grant, and change
...
destroy](/reference/security-model#revoking-and-rotating-a-key).
...
<Warning>
  ...
  chip. [Set one at mint time](/start/first-key#expiry).
</Warning>
...
<Note>
  ...
  MCP_HTTP_PORT on the box and restart it.` — or, on the screen after a mint, `This
...
**Connect claude.ai.** In claude.ai, open **Settings** → **Connectors** → **Add
custom connector**. Paste the address you copied into the field for the server
address. Leave **Client ID** and **Client Secret** empty. Postern issues no client
secret, and it registers the agent itself.

<Note>
  **Nobody has captured two labels in claude.ai's own dialog, so this page does not
  print them.** We do not know what it calls the address field. We do not know whether
  it ever asks how you want to sign in. One run, on 2026-07-21, is the whole record:
  the dialog offered **Client ID** and **Client Secret**, the founder left both empty,
  and the connection worked. The Console's own copy says to pick **OAuth** if the
  agent asks how to authenticate. One screenshot of that dialog settles both.
</Note>

claude.ai then opens a page served by your own Postern, headed **Authorize agent
...
page turns on with the address; there is no separate switch. [Hosted agents and the
OAuth bridge](/reference/mcp-primitives#hosted-clients-and-the-oauth-bridge).
...
- You type `What context can you reach?` and the agent answers with the sectors you
  ticked for that key, how fresh each connection is, and nothing outside them.
...
under its name, and Postern only ever adds to the record. [What gets a
row](/reference/audit-log#what-gets-a-row). What you do to the key is not there:
create a key, rotate one, edit its sectors or its deadline, revoke it or remove it,
and Postern writes no row at all. [What does not get a
row](/reference/audit-log#what-does-not-get-a-row).

Your config file now holds a credential. Store it where your other passwords live.
[Agent keys](/reference/security-model#agent-keys).
