...
<Info>
  **Before you start**
  ...
  - **A Mac or a Linux computer.** Windows should work, because Postern is Docker, but nobody has run this page on Windows. In PowerShell, `curl` means something else, so type `curl.exe` in the checks below.
</Info>
...
last-command-wins per port, so a mistyped command on any machine that holds the
...
This page does not walk Tailscale's policy file. If the permission is missing,
...
Typing does not save it. Press **Publish this gate** on the bar headed **Publish
this gate to the public internet?**, which lists what it costs you. The row then
reads **restart to apply**, and a message reads
`Saved — restart the gateway to start publishing.`

<Frame caption="Settings → Remote access. The Public row reads off until you set an address.">
...
address. The dialog also offers a **Client ID** and a **Client Secret**: leave
both empty. claude.ai then sends you to Postern's own page, headed **Authorize
agent access**. Paste the agent key under **Paste an agent key from the Console**
and press **Approve**. claude.ai holds a token that rotates from then on.
ChatGPT takes a longer path: [Connect ChatGPT](/connect/chatgpt).
...
Confirm nothing is left. The output must contain no `AllowFunnel` anywhere:
...
| The **Public** row still reads **off** after you typed the address | Typing does not save it. Press **Publish this gate** on the confirmation bar ([step 4](#publish)). |
...
| A hosted agent says it could not reach you, and your logs show nothing | The funnel is on a port other than 443, so the connection is dropped before it arrives. Run `tailscale funnel status`; the 443 line must read `(Funnel on)`. |
